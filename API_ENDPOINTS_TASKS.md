...
```json
{
  ...
  "description": "string (optional)",
  "icon": "string (optional) - one of: task, bug, feature, improvement, question",
...
```javascript
{
  ...
  icon: String (required, enum: ["task", "bug", "feature", "improvement", "question"]),
...
4. **Icon Values:** The icon field accepts one of these predefined values:
   - `"task"` - 📋 Task
   - `"bug"` - 🐛 Bug
   - `"feature"` - ✨ Feature
