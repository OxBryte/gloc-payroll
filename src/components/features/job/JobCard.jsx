      ...
      <div className="w-full bg-white rounded-lg border border-gray-100 p-4 flex flex-col gap-3 hover:border-c-color/50">
        ...
        <div className="flex items-center gap-3 justify-between w-full">
          ...
          <button
            ...
            className="w-fit px-5 py-2 text-white bg-c-color text-sm cursor-pointer hover:bg-c-bg rounded-lg"
          >
            View Details
          </button>
        </div>
      </div>
      {isModalOpen && <JobDetailsModal job={job} setIsOpen={setIsModalOpen} />}
    </>
  );
}
